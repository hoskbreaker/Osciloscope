					...
					else
					{
						...
						if ((cChar == 'A') || (cChar == 'B'))
						{
							...
							for (x = 0; g_pcUARTRxHistoryBuffer[copyFrom] != '\0'; x++)
							{
								...
								UARTwrite((const char *) &g_pcUARTRxBuffer[g_ui32UARTRxWriteIndex], 1);
								ADVANCE_RX_BUFFER_INDEX(g_ui32UARTRxWriteIndex);
								if (copyFrom >= UART_RX_HISTORY_BUF_SIZE)
									copyFrom = 0;
							}
							g_pcUARTRxBuffer[g_ui32UARTRxWriteIndex] = '\0';
                    ...
                    UARTwrite("\n", 1);
						...
						if (RX_BUFFER_USED > 0)
						{
							...
							if (g_bUARTRxHistoryCnt > 0)
							{
								...
								x = g_ui32UARTRxReadIndex;
								count = GetBufferCount(&x, &g_ui32UARTRxWriteIndex, UART_RX_BUFFER_SIZE);
